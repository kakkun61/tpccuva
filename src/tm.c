...
EXEC SQL BEGIN; /*COMIENZO DE TRANSACCION*/
	...
	if (sqlca.sqlcode<0){
		...
		fprintf(ferr, "      %s\n", sqlca.sqlerrm.sqlerrmc);
		fprintf(ferr, "VALORES DE LOS CAMPOS:\n");
		fprintf(ferr, "o_id = %d, d_id = %d, w_id = %d, c_id = %d, o_entry_d = %p, o_all_local = %d\n", o_id, d_id, w_id, c_id, o_entry_d, o_all_local);
		EXEC SQL ROLLBACK;
		shm->new_order.o_ol_cnt=o_ol_cnt;
		return -1;
	} /* if */
...
fprintf(f_res_deli, "Inicio Transaccion. Fecha Encolado: %d %d, Almacen %d, Repartidor %d\n", (int) delivery->seg, 
      (int) delivery->mseg, delivery->w_id, delivery->o_carrier_id);
	  ...
	  if (salir == 0) {
		...
		break;
	      ...
	      case DELIVERY: /*******TRANSACCIÓN DELIVERY******/
			...
			ftime(&sellohora);
			fprintf(f_tr_deli,"\n %d %d %d %d %d %d ",men.tran.delivery.w_id, men.tran.delivery.d_id, 
                              (int) men.tran.delivery.seg, (int) men.tran.delivery.mseg, (int) sellohora.time,
                              (int) sellohora.millitm);
			/*Se escribe en el fichero de resultados el sollo de hora de finalización*/
			fprintf(f_res_deli, "Terminada a las: %d %d\n\n", (int)sellohora.time, (int)sellohora.millitm);
