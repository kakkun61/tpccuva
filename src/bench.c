...
initstate(semilla++, estados[E_W_CITY],32); /*seed (semilla) = 6*/
  ...
  for (i=1; i <= w; i++){
	...
	// v1.2.4: Commit delayed to speed up process.
        // EXEC SQL COMMIT;
...
initstate(semilla++, estados[E_D_TAX],32); /*seed (semilla) = 11*/
  ...
  for (i = 1; i <= w; i++){ /*for each warehouse */
	...
	/**************ERROR HANDLING********/
	if (sqlca.sqlcode<0){
		fprintf(stdout, "ERROR INSERTING A ROW.\n");
		fprintf(stdout, "ERROR TYPE: %ld\n", sqlca.sqlcode);
		fprintf(stdout, "      %s\n", sqlca.sqlerrm.sqlerrmc);
	...
	/**************ERROR HANDLING********/
	if (sqlca.sqlcode<0){
		fprintf(stdout, "ERROR DURING COMMIT\n");
		fprintf(stdout, "ERROR TYPE: %ld\n", sqlca.sqlcode);
		fprintf(stdout, "      %s\n", sqlca.sqlerrm.sqlerrmc);
		fprintf(stdout, "ITEM %d, ALMACEN %d\n", j+1, i);
	} /* if */
...
/* SQL variables */
 ...
 /* initializing state vectors */ 
  ...
  for (i=1; i <= n; i++){ /* for each warehouse */
	...
	// Change 1.2.4 Delaying commiet to speed up performance.
        // EXEC SQL COMMIT;
...
/* Initializing state vectors */
  ...
  for (i = 1; i <= w; i++){ /*for each warehouse*/
	...
	for (z = 1; z <= NUM_CLIENT; z++){ 
		...
		// change 1.2.4: Delaying commit to speed up performance.
                // EXEC SQL COMMIT; 
  ...
  for (i = 1; i <= w; i++){ /*for each warehouse*/
	...
	for (z = N_OR_UNDELIVER; z <= NUM_ORDER; z++){ 
		...
		EXEC SQL INSERT 
	 		  INTO new_order (no_o_id, no_d_id, no_w_id)
				VALUES (:no_o_id, :no_d_id, :no_w_id);
		// new in 1.2.4: delaying commit to speed up process.
                // EXEC SQL COMMIT; 
		/************ERROR HANDLING********/
		if (sqlca.sqlcode<0){
			fprintf(stderr, "ERROR INSERTING THE ROW IN THE ORDERR TABLE\n");
...
/* Populating the database */
	...
	creartablas();
        // new in 1.2.4: only one commit per table.
        EXEC SQL COMMIT;
...
strcat(filenameBuffer,"check.log");
	...
	fscanf(aux, "1º sello %d %d 2º sello %d %d", (int*)&sellohora1.time, (int*)&sellohora1.millitm, (int*)&sellohora2.time, (int*)&sellohora2.millitm);
	while(!feof(aux)){
		...
		/* Reading next line */
		fscanf(aux, " \n1º sello %d %d 2º sello %d %d", (int*)&sellohora1.time, (int*)&sellohora1.millitm, (int*)&sellohora2.time, (int*)&sellohora2.millitm);
	} /* while */
	fclose(fich);
	fclose(aux);
	} /* if */
...
strcat(filenameBuffer,"medida.log");
			...
			if (med != NULL){
				...
				fprintf(med,"Measurement interval start timestamp: %d %d .\n"
                                      ,(int)sellohora2.time, (int)sellohora2.millitm);
			}
